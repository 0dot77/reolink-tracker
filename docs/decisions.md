...
## 2026-05-12: 야간은 RGB/Color 입력을 우선한다
...
결정: 야간 실험에서는 Auto IR/B&W 전환과 RGB/Color 입력을 구분해서 기록하고, 가능하면
RGB/Color 입력을 우선합니다. 스폿라이트나 ColorX는 이상적인 입력 안정화 수단이지만,
현재 현장에서는 스폿라이트를 켤 수 없으므로 다음 결정의 config-only 실험을 우선합니다.
노출/게인/day-night threshold는 야간 시간대에서 한 번 튜닝합니다.

결과: 코드 변경 없이 가능한 입력 안정화 방향을 정하되, 현장 제약 때문에 즉시 실행 계획은
`docs/cam2-low-light-research.md`의 `_sub + imgsz 1280 + yolo26s.pt` baseline 검증으로
수정합니다.

## 2026-05-12: 스폿라이트 없이 `imgsz`와 모델 크기를 먼저 올려 검증한다

현장에서는 스폿라이트를 켤 수 없으므로, 위 ColorX + spotlight 운용 가정은 즉시 적용 가능한
해결책이 아닙니다. 이전 baseline은 `_sub` stream + `yolo26n.pt` + `imgsz: 640` + 낮은 raw
confidence 조합이라, 40m 중앙부의 작은 옆모습 실루엣을 놓치는 것이 자연스럽습니다.

결정: main stream이나 cam2 auxiliary 작업으로 바로 넘어가기 전에, reversible config-only
변경으로 `imgsz: 1280`과 `model: yolo26s.pt`를 현장 기본 후보로 올려 한 사이클 검증합니다. stream은 우선
`_sub`를 유지합니다. FPS가 15 아래로 떨어지거나 OSC/ID latency가 나빠지면 `imgsz: 960`으로
되돌려 비교합니다.

결과: 1차 실험의 attribution을 깨끗하게 유지합니다. 이 조합으로 중앙 side-profile detection이
살아나면 2카메라 구성에서 threshold만 최소 조정하고, 부족하면 `docs/cam2-auxiliary-direction.md`
의 auxiliary confirmer 작업으로 넘어갑니다. `_main` stream은 H.265/decode/bandwidth 리스크가
있으므로 첫 실험에서 제외합니다.
...
`preprocessing.clahe.{enabled, clip_limit, tile_grid, cameras}`로 두고, 현재 현장 기본은
`clip_limit: 2.0`, `tile_grid: [8, 8]`, `cameras: [cam2]`입니다. cam0/cam1 primary baseline은
건드리지 않고 중앙 auxiliary 카메라에만 적용합니다.

결과: 야간 측면/후방 검출이 보강되며, 코드 surface는 한 카메라 worker 안에 갇혀 있어
fusion이나 OSC schema에 영향이 없습니다. 2026-05-12 replay에서는 cam2 accepted auxiliary
event가 2개에서 4개로 늘었지만 `auxiliary_center_event_samples`는 여전히 0이었습니다.
따라서 CLAHE는 약간의 보강일 뿐, 현재 클립의 중앙 보행자 문제를 단독 해결하지는 못합니다.
...
`docs/cam2-auxiliary-direction.md`. 이 필드와 fusion 키는 구현되었고, Tauri UI도
`primary`/`auxiliary` 역할을 구분해서 보여줍니다.

2026-05-12 replay 검증: `~/Desktop/vom-reolink-videosim/live-20260511-204813-20s-usage`
의 기존 `cam0/cam2/cam1` 20초 클립을 current runtime config(`yolo26s.pt`, `imgsz: 1280`,
`cam2 role: auxiliary`)로 재처리했습니다. 초기 결과는 400 frame 중 accepted event가
`cam0=144`, `cam1=1`, `cam2=0`, auxiliary event가 0이었습니다. cam2 전용
`conf: 0.08`, `body_catch_min_confidence: 0.08`, `auxiliary_confirm_hits: 1`로 바꾸면
cam2 auxiliary event가 2개 생겼고, 여기에 cam2 전용 CLAHE를 켜면 4개까지 늘었습니다.
그러나 `auxiliary_center_event_samples`는 계속 0이었습니다. 즉 auxiliary fusion 로직은
준비되었지만, 해당 저조도 클립에서는 cam2 중앙 보행자가 충분한 sighting을 만들지 못해
handoff 보강 효과가 제한적입니다. 다음 판단은 auxiliary radius 조정보다 cam2 입력/검출
개선(Color/RGB 고정, ROI, site fine-tune) 쪽입니다.
...
`models/site/best.pt`에 두고 `.gitignore`에 `models/`를 추가합니다. 현재는 top-level
`model` 또는 `--model`로 이 weight를 지정하고, 별도 `model_path` key는 필요해질 때만
도입합니다.
