...
크기/비율 필터와 confirm window는 그대로 통과해야 합니다.
...
기준을 적용하는 별도 마스크입니다. 이 경로에서도 confirm window는 유지됩니다. 위치는
기존 homography에서 나온 `u`를 우선 쓰고, `relaxed_presence_v`가 있으면 해당 projection
v로 고정합니다. 없으면 `v`만 projection 범위 안으로 clamp하므로 TouchDesigner OSC payload
순서는 바뀌지 않습니다. 설정 alias로 `stair_catch_points`도 읽을 수 있지만 저장할 때는
...
`relaxed_presence_points`로 들어가며, UV homography를 새로 만들지 않고 기존
`image_points`의 `u` 위치만 사용합니다. 앱 캘리브레이션 화면에서는 계단 전용
